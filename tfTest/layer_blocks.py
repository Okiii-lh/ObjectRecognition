...
def conv2d(x, filter_size, num_filters, strides, name, 
    ...
    convolve = lambda i, j: tf.nn.conv2d(i, j, 
    ...
    # In the cases of multiple groups, split inputs & weights and
        ...
        x_groups = tf.split(axis=3, num_or_size_splits=groups, 
        	value=x)
        W_groups = tf.split(axis=3, num_or_size_splits=groups, 
        	value=W)
        output_groups = [convolve(i, k) for i, k in zip(x_groups, 
        	W_groups)]
...
def conv2d_fixed_padding(x, filter_size, num_filters, strides, 
	name, add_bias=True):
    ...
    def fixed_padding():
        ...
        padded_x = tf.pad(x, [[0, 0], [pad, pad], [pad, pad], 
        	[0, 0]]) # [batch, height, width, channel]
        return padded_x
    ...
    return tf.identity(conv + b if add_bias 
    	else conv, name=scope.name)
...
def batch_norm(x, num_filters, name):
    ...
    return tf.nn.batch_normalization(x, mean=m, 
    	variance=v, scale=s, offset=o, variance_epsilon=1e-5, name=name)

def batch_normalization(x, is_training, name):
	# TODO 没有用到
    return tf.layers.batch_normalization(x,axis=-1,momentum=0.99, 
    	epsilon=1e-3, center=True,scale=True, training=is_training, 
    	name=name)
...
def residual_block(x, num_filters, strides, is_training, name):
    ...
    ## Block A
    # branch2a
    conv_a2a = conv2d_fixed_padding(x, filter_size=[3,3], 
    	num_filters=num_filters, strides=strides, 
    	name='res'+name+'a_branch2a', add_bias=False)
    bn_a2a = batch_norm(conv_a2a, num_filters=num_filters, 
    	name='bn'+name+'a_branch2a')
    relu_a2a = tf.nn.relu(bn_a2a, name='res'+name+'a_branch2a_relu')
    
    # branch2b
    conv_a2b = conv2d_fixed_padding(relu_a2a, 
    	filter_size=[3,3], num_filters=num_filters, 
    	strides=1, name='res'+name+'a_branch2b', add_bias=False)
    bn_a2b = batch_norm(conv_a2b, num_filters=num_filters, 
    	name='bn'+name+'a_branch2b')
...
## 转换块的不同版本## Different versions of transformation blockss
#tf.layers和tf.nn作用基本相同，
...
def transformation_block(x, num_filters, kernel_constraint, training_phase, name):
    ...
    #use_bias: 该层是否使用偏差, kernel_constraint: 在权重上施加约束
