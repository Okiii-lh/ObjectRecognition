...
def update_collection(name, feature, collection):
    ...
    # if it's the first feature to be inserted in the collection
    if np.shape(collection[name])[0] == 0:
...
def collect_features(sess, feed_dict, nodes, labels, collection):
    ...
    # Generate collection dictionary to store all features if 'collection is
    # empty
    if collection == {}:
        for node in nodes:
...
def log_file(history_callback, log_dir, params):
    """
    TODO 日志文件 12月11日 代码修改
    该方法在源代码中没有使用到 并且方法有错误 将报错处注释掉
